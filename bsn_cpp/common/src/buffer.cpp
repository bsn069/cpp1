...
C_Buffer::C_Buffer(NBsn::I_Alloc::T_SPI_Alloc spI_Alloc, uint32_t u32InitSize)
    : m_pData(nullptr)
    , m_u32Capacity(0)
    , m_u32WritePos(0)
...
bool C_Buffer::Realloc(uint32_t u32Len) {
...
uint8_t* C_Buffer::GetWritePtr() {
    return (m_pData + m_u32WritePos);
}

uint8_t* C_Buffer::GetReadPtr() {
    return m_pData;
}

uint32_t C_Buffer::GetCapacity() {
    return m_u32Capacity;
}

uint32_t C_Buffer::GetCanReadLength() {
    return m_u32WritePos;
}

uint32_t C_Buffer::GetCanWriteLength() {
    return (GetCapacity() - m_u32WritePos);
}

bool C_Buffer::Read(uint32_t u32Len) {
	D_Assert(u32Len > 0);
    D_Assert(m_spI_Alloc);
...
bool C_Buffer::Write(uint8_t const* pBuf, uint32_t u32Len) {
	D_Assert(u32Len > 0);
